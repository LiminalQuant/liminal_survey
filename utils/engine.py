...
def check_password():
    ...
    st.markdown("---")
    st.warning("⚠️ Только для администратора")
    if st.button("🧹 Очистить все данные опроса"):
        import os
        from .storage import DATA_PATH
        if DATA_PATH.exists():
            os.remove(DATA_PATH)
            st.success("✅ Данные удалены. Файл responses.csv пересоздастся при следующем ответе.")
        else:
            st.info("Файл уже отсутствует.")
# ==========================
